...
delete from user_credentials where customer_id ='CUSVI97159';
...
delete from user_data where customer_id ='CUSVI97159';
...
delete from user_address where customer_id ='CUSVI97159';
...
select services.service_id, service_requests.device_type, service_requests.defect, services.service_date from services, service_requests where services.service_status = 'Initiated' and services.service_id = service_requests.service_id and services.serviced_by = null;
-- select service_id, device_type, model, defect ,usaage, repair_status, price, payment_status, time from service_requests where time = (select max(time) from service_requests where customer_id = 'CUSAG90800');

select services.service_id, service_requests.device_type, service_requests.defect, services.service_date 
from services, service_requests 
where services.service_id = (select service_id from services where serviced_by is null) and services.service_id = service_requests.service_id;

delete from services where service_id = null